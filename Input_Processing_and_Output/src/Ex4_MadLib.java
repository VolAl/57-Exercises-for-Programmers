...
public class Ex4_MadLib {
    ...
    public static void main(String[] args) {
        ...
        if(!branchingStoryElements.isEmpty()) {
            ...
            while(true) {
                ...
                List<String> verbValues = branchingStoryElements.get(VERB);
                List<String> adjectiveValues = branchingStoryElements.get(ADJECTIVE);
                List<String> nounValues = branchingStoryElements.get(NOUN);
                List<String> adverbValues = branchingStoryElements.get(ADVERB);

                switch (category) {
                    case 0:
                        System.out.println(MessageFormat.format("You can always {0} your {1} {2} {3}!",
                    ...
                    case 1:
                        ...
                        break;
                    case 2:
                        System.out.println(MessageFormat.format("You can always {3} your {2} {1} {0}!",
                                verbValues.isEmpty() ? "" : verbValues.get(new Random().nextInt(verbValues.size())),
                                adjectiveValues.isEmpty() ? "" : adjectiveValues.get(new Random().nextInt(adjectiveValues.size())),
                                nounValues.isEmpty() ? "" : nounValues.get(new Random().nextInt(nounValues.size())),
                                adverbValues.isEmpty() ? "" : adverbValues.get(new Random().nextInt(adverbValues.size()))));
    ...
    private static Map<String, List<String>> gatherBranchingStoryElements(Scanner sc) {
        ...
        while (true) {
            ...
            int category = JOptionPane.showOptionDialog(f, "Specify a Category or click \"EXIT\" to stop.", "Let's expand the story!",
